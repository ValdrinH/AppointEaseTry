...
namespace AppointEase.AspNetCore.Controllers
{
    ...
    public class RequestConnectionController : ControllerBase
    {
        ...
        public async Task<IActionResult> CheckConnection([FromQuery] string userId, [FromQuery] string doctorId)
        {
            var result = await _connectionRequestService.CheckConnectionIfExist(userId, doctorId);
            return Ok(result);
        }
